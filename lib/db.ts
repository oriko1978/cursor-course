...
export const database = {
  getAll(): ApiKey[] {
    const stmt = db.prepare(`
      SELECT id, name, key, type, monthly_limit, created_at, last_used, is_active
      FROM api_keys
  ...
  create(data: {
    ...
    const stmt = db.prepare(`
      INSERT INTO api_keys (id, name, key, type, monthly_limit, created_at, is_active)
      VALUES (?, ?, ?, ?, ?, ?, 1)
    `);
    
    stmt.run(id, data.name, key, data.type, data.monthlyLimit || null, now);
